...
class Dependencies {
  ...
  instanceOf(Type type, [Type annotation]) {
    ...
    if (injector == null) throw new StateError("injector is not initialised");
  ...
  instanceCalled(String name) {
    ...
    if (!typemap.containsKey(name)) throw new ArgumentError("no type registered for $name");

    return instanceOf(typemap[name]);

  }
  
  typeCalled(String name) {
